...
export const ARTIFACT_GUARDRAIL =
  "ABSOLUTELY NO TEXT OR ATTRIBUTION OF ANY KIND ANYWHERE on the page. This includes: no model name (NO 'Nano', 'Banana', 'Gemini', 'AI', 'Generated by', or any vendor/product attribution text — the model must NEVER stamp its own name on the image), no letters, no numbers, no labels, no speech bubbles, no watermarks (visible or stylized), no signatures, no logos, no captions, no frames-within-frames, no page numbers, no signature dots, no glassy/translucent text overlays in any corner. Exception: when the page subject itself explicitly requires a readable letter or number as the main educational object, render only that requested character and keep every other text mark forbidden. Scan the four corners and bottom strip of the canvas for stray attribution text and ensure they are completely empty of letters. A single coherent illustration with ZERO text marks except a subject-requested educational character.";
...
export const NO_AI_BORDER_RULE =
  "RULE #1 — READ THIS FIRST AND OBEY. The page MUST BE BORDERLESS. Do NOT draw a rectangular border. Do NOT draw a page outline. Do NOT draw a printer's frame. Do NOT draw a thin black rectangle at 3% / 4% / 5% / any inset from the page edges. Do NOT draw ANY rectangle, frame, outline, or printer's mark anywhere — even faintly, even in the corners. The page is COMPLETELY BORDERLESS from your side. You are seeing coloring books in training data that have borders, but THIS BOOK DOES NOT: the printer's vector border is added in POST-PROCESSING after you finish, so any border you draw produces a DOUBLE BORDER on the printed page. The illustration content fills the canvas edge-to-edge with NOTHING else at the edges. Before submitting the image, scan the four edges and four corners; if you see ANY straight line forming a rectangle near the edges, REMOVE IT. Only the line art of the actual scene/subject content. NO frame. NO border. NO outline rectangle. ZERO.";
