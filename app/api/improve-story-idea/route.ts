// Improve the user's short story-book idea into a richer planner-ready
...
import { IMPROVE_STORY_IDEA_SYSTEM } from "@/lib/prompts/idea-improve";
...
export async function POST(req: Request) {
  ...
  if (body.storyType)
    contextLines.push(`Story type: ${userInput(body.storyType)}`);
  if (body.dialogueStyle)
    contextLines.push(`Dialogue style: ${userInput(body.dialogueStyle)}`);
  if (body.characterNames)
    contextLines.push(
      `Character names the user has in mind: ${userInput(body.characterNames)}`,
    );

  const context =
    contextLines.length > 0 ? `\n\nContext:\n${contextLines.join("\n")}` : "";
  const userPrompt = `User's idea:\n${userInput(idea)}${context}\n\nRewrite the idea into a richer brief per your instructions. Output only the rewritten paragraph.`;

  try {
    const result = await generateText({
      model: openai(OPENAI_TEXT_MODEL),
      system: IMPROVE_STORY_IDEA_SYSTEM,
      prompt: userPrompt,
    });
